...
export default function AdminCakeOrders() {
  ...
  const handleSoftDelete = async (id) => {
    ...
    const { error } = await supabase.from('cake_orders').update({ admin_hidden: true }).eq('id', id)
    if (!error) fetchOrders(page)
  }

  const handlePermanentDelete = async (id) => {
    if (!confirm('Permanently delete this order? This removes it everywhere, including the customer\'s My Orders. This cannot be undone.')) return
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-16">
      <div className="mb-10">
        ...
        <p className="text-gray-500 text-sm mt-1">{totalCount} total request{totalCount !== 1 ? 's' : ''}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border border-dashed border-gray-300 rounded-2xl">
          ...
          <p className="text-gray-500">No cake order requests yet.</p>
        </div>
      ) : (
        <>
            ...
            {orders.map((order) => (
              <div key={order.id} className="border border-gray-200 rounded-2xl p-6">
                ...
                <div className="grid sm:grid-cols-3 gap-4 text-sm mb-4">
                  <div>
                    <p className="text-gray-400 text-xs uppercase font-medium">Flavor</p>
                    <p className="text-gray-700">{order.flavor || '—'}</p>
                  </div>
                  <div>
                    <p className="text-gray-400 text-xs uppercase font-medium">Size</p>
                    <p className="text-gray-700">{order.size || '—'}</p>
                  </div>
                  <div>
                    <p className="text-gray-400 text-xs uppercase font-medium">Delivery Date</p>
