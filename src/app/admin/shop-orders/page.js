...
const fulfillmentOptions = ['processing', 'shipped', 'delivered', 'cancelled']
const PAGE_SIZE = 10
...
export default function AdminShopOrders() {
  ...
  const updateFulfillment = async (id, status) => {
    const { error } = await supabase
      .from('orders')
      .update({ fulfillment_status: status })
      .eq('id', id)
    if (!error) fetchOrders(page)
  }

  const handleSoftDelete = async (id) => {
    if (!confirm('Remove this from your dashboard? The customer will still see it in their My Orders.')) return
    const { error } = await supabase.from('orders').update({ admin_hidden: true }).eq('id', id)
    if (!error) fetchOrders(page)
  }

  const handlePermanentDelete = async (id) => {
    if (!confirm('Permanently delete this order? This removes it everywhere, including the customer\'s My Orders and Transactions. This cannot be undone.')) return
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-16">
      <div className="mb-10">
        ...
        <p className="text-gray-500 text-sm mt-1">{totalCount} total order{totalCount !== 1 ? 's' : ''}</p>
      </div>
          ...
          <div className="grid gap-5 mb-8">
            {orders.map((order) => (
              <div key={order.id} className="border border-gray-200 rounded-2xl p-6">
                ...
                <div className="space-y-1 mb-4 bg-gray-50 rounded-xl p-4">
                ...
                <div className="grid sm:grid-cols-3 gap-4 text-sm mb-4">
                  <div>
                    <p className="text-gray-400 text-xs uppercase font-medium">Delivery Zone</p>
                    <p className="text-gray-700">{order.delivery_zone || '—'}</p>
                  </div>
                  <div>
                    <p className="text-gray-400 text-xs uppercase font-medium">Delivery Fee</p>
                    <p className="text-gray-700">₦{Number(order.delivery_fee).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-gray-400 text-xs uppercase font-medium">Total</p>
                    <p className="text-gray-900 font-semibold">₦{Number(order.total).toLocaleString()}</p>
                  </div>
                </div>

                <p className="text-xs text-gray-400 mb-4">
                  Ref: {order.payment_reference} · Ordered {new Date(order.created_at).toLocaleString()}
                </p>
